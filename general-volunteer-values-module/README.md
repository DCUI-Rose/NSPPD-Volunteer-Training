...
```
https://yourname.github.io/volunteer-trainings/general-volunteer-values/
...
5. General Volunteer Values itself is live at:
   `https://yourname.github.io/volunteer-trainings/general-volunteer-values/`

---

## Part B — Connect all six trainings to one Google Sheet

All six trainings share **one Google Sheet** (with a separate tab per training) and **one Apps Script deployment**. Full walkthrough is in [`apps-script-setup.md`](apps-script-setup.md). Short version:

1. Create a Google Sheet with a tab for each training: **General Volunteer Values**, **Crowd Medics**, **Roaming Medics**, **Group Lead**, **Coordinator**, **Testimony Vetting** — each with headers `Timestamp | Name | Volunteer ID | Phone | Score | Total`.
...
4. In **each** of the six `index.html` files, find:
   ```javascript
   const SCRIPT_URL = "REPLACE_WITH_YOUR_APPS_SCRIPT_WEB_APP_URL";
   ```
   and paste in the **same** URL — all six files get an identical line.
5. Re-upload those six files to GitHub (or edit each directly with the pencil icon) and commit.
