# Google Sheets setup — one Sheet, one Script, six tabs

All six trainings write to **the same Google Sheet**, each into its own tab, using **one Apps Script deployment** shared by every training page.

## 1. Create the sheet

1. Create a new Google Sheet, e.g. **"Volunteer Training — Results"**.
2. Rename the default first tab (bottom-left) to **General Volunteer Values**.
3. Add five more tabs (right-click the tab bar → Insert sheet), named exactly:
   - **Crowd Medics**
   - **Roaming Medics**
   - **Group Lead**
   - **Coordinator**
...
## 3. Wire it into all six training pages

Each training already sends its own tab name automatically (`General Volunteer Values`, `Crowd Medics`, `Roaming Medics`, `Group Lead`, `Coordinator`, `Testimony Vetting`) — you don't need to configure that part. You just need to paste the **same** Web app URL into all six files.

In each of `general-volunteer-values/index.html`, `crowd-medics/index.html`, `roaming-medics/index.html`, `group-lead/index.html`, `coordinator/index.html`, and `testimony-vetting/index.html`, find:

```javascript
const SCRIPT_URL = "REPLACE_WITH_YOUR_APPS_SCRIPT_WEB_APP_URL";
```

and replace the placeholder with the **same** Web app URL you copied in step 2 — every training gets an identical line. Commit and push all six files.
